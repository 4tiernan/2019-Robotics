...
public:
  ...
  int8_t getBlocks(bool wait=false, uint8_t sigmap=CCC_SIG_ALL, uint8_t maxBlocks=0xff);//wait was true
...
template <class LinkType> int8_t Pixy2CCC<LinkType>::getBlocks(bool wait, uint8_t sigmap, uint8_t maxBlocks)
{
  ...
  while(1)
  {
    ...
    if (m_pixy->recvPacket()==0)
    {
      ...
      {
        if (!wait){ //if wait = 0 then exit pixy stuff
